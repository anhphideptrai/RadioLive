...
static NSString *const kLCDSymbolDotPositionTop          = @"LCDDotTop";
static NSString *const kLCDSymbolDotPositionBottom       = @"LCDDotButtom";
         ...
         inContext:(CGContextRef)ctx
{
    ...
    if ([[partsMap valueForKey:kLCDSymbolDotPositionTop] boolValue])
    {
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathMoveToPoint(path, NULL, startPoint.x, startPoint.y + (edgeLength - lineWidth)/2);
        CGPathAddLineToPoint(path, NULL, startPoint.x, startPoint.y + (edgeLength - lineWidth)/2 +  lineWidth);
        CGPathAddLineToPoint(path, NULL, startPoint.x + lineWidth, startPoint.y + (edgeLength - lineWidth)/2 +  lineWidth);
        CGPathAddLineToPoint(path, NULL, startPoint.x + lineWidth, startPoint.y + (edgeLength - lineWidth)/2);
        CGPathCloseSubpath(path);
        CGContextAddPath(ctx, path);
        CGContextFillPath(ctx);
        CGPathRelease(path);
    }
    if ([[partsMap valueForKey:kLCDSymbolDotPositionBottom] boolValue])
    {
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathMoveToPoint(path, NULL, startPoint.x, startPoint.y + (edgeLength - lineWidth)/2 + edgeLength + spacer);
        CGPathAddLineToPoint(path, NULL, startPoint.x, startPoint.y + (edgeLength + lineWidth)/2 + edgeLength + spacer);
        CGPathAddLineToPoint(path, NULL, startPoint.x + lineWidth, startPoint.y + (edgeLength + lineWidth)/2 + edgeLength + spacer);
        CGPathAddLineToPoint(path, NULL, startPoint.x + lineWidth, startPoint.y + (edgeLength - lineWidth)/2 + edgeLength + spacer);
        CGPathCloseSubpath(path);
        CGContextAddPath(ctx, path);
        CGContextFillPath(ctx);
    ...
    switch (symbol) {
        case FBFontSymbol0:
            return [NSDictionary dictionaryWithObjectsAndKeys:
                ...
                [NSNumber numberWithBool:NO], kLCDSymbolDotPositionTop,
                [NSNumber numberWithBool:NO], kLCDSymbolDotPositionBottom,
                nil];
        case FBFontSymbol1:
            return [NSDictionary dictionaryWithObjectsAndKeys:
        ...
        case FBFontSymbolColon:
            return [NSDictionary dictionaryWithObjectsAndKeys:
                    ...
                    [NSNumber numberWithBool:NO], kLCDSymbolPartPositionRightBottom,
                    [NSNumber numberWithBool:NO], kLCDSymbolPartPositionCenter,
                    [NSNumber numberWithBool:YES], kLCDSymbolDotPositionTop,
                    [NSNumber numberWithBool:YES], kLCDSymbolDotPositionBottom,
                    nil];
        default:
            return [NSDictionary dictionaryWithObjectsAndKeys:
                    [NSNumber numberWithBool:NO], kLCDSymbolPartPositionTop,
